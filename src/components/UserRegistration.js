
import React, { useState } from "react";
import { useDispatch } from "react-redux";
import { Link } from "react-router-dom";
import { 
  setUsername,
  setAddress,
  setContactnumber,
  setEmail,
  setPassword,
  registerUserData
} from "../redux/userRegistrationSlice";
import { Form, Button, Card } from 'react-bootstrap';

const UserRegistration = () => {
  const dispatch = useDispatch();

  const [formData, setFormData] = useState({
    user_name: "",
    address: "",
    contact_number: "",
    email: "",
    password: "",
  });

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData({ ...formData, [name]: value });
    // Dispatch actions to store individual form fields
    switch (name) {
      case "user_name":
        dispatch(setUsername(value));
        break;
      case "address":
        dispatch(setAddress(value));
        break;
      case "contact_number":
        dispatch(setContactnumber(value));
        break;
      case "email":
        dispatch(setEmail(value));
        break;
      case "password":
        dispatch(setPassword(value));
        break;
      default:
        break;
    }
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    // Basic validation
    if (
      formData.user_name.trim() === "" ||
      formData.address.trim() === "" ||
      formData.contact_number.trim() === "" ||
      formData.email.trim() === "" ||
      formData.password.trim() === ""
    ) {
      alert("Please fill in all fields.");
    } else {
      dispatch(registerUserData(formData));
    }
  };

  return (
    <Card className="mx-auto mt-5 p-4" style={{ width: '400px' }}>
      <Card.Title className="text-center mb-4" style={{ fontSize: '28px' }}>Register</Card.Title>
      <Form onSubmit={handleSubmit}>
        <Form.Group controlId="formUserName">
          <Form.Label>Name:</Form.Label>
          <Form.Control
            type="text"
            placeholder="Enter name"
            name="user_name"
            value={formData.user_name}
            onChange={handleChange}
          />
        </Form.Group>
        
        {/* Other form groups */}
        <Form.Group controlId="formAddress">
          <Form.Label>Address:</Form.Label>
          <Form.Control
            as="textarea"
            rows={3}
            placeholder="Enter address"
            name="address"
            value={formData.address}
            onChange={handleChange}
          />
        </Form.Group>
        <Form.Group controlId="formContactnumber" style={{marginTop:'10px'}}>
            <Form.Label>Contact_number:</Form.Label>
            <Form.Control
              type="text"
              placeholder="Enter contact_number"
              name="contact_number"
              value={formData.contact_number}
              onChange={handleChange}
            />
          </Form.Group>
          <Form.Group controlId="formEmail" style={{marginTop:'10px'}}>
            <Form.Label>Email:</Form.Label>
            <Form.Control
              type="email"
              placeholder="Enter email"
              name="email"
              value={formData.email}
              onChange={handleChange}
            />
          </Form.Group>
          <Form.Group controlId="formPassword" style={{marginTop:'10px'}}>
            <Form.Label>Password:</Form.Label>
            <Form.Control
              type="password"
              placeholder="Enter password"
              name="password"
              value={formData.password}
              onChange={handleChange}
            />
          </Form.Group>
        {/* Add other form groups for contact_number, email, and password */}
        {/* ... */}

        <Button variant="primary" type="submit" className="mt-3">
          Register
        </Button>
      </Form>

      <div className="text-center mt-3">
        <p className="mb-0">Already have an account? <Link to="/SellerLogin">Sign In</Link></p>
      </div>
    </Card>
  );
};

export default UserRegistration;
